<?php
/**
 * BaseTemplate class for the NanoCenter skin
 *
 * @ingroup Skins
 */
class NanoCenterTemplate extends BaseTemplate {
	...
	public function execute() {
		...
		<a href="https://nanocenter.org/about">About Us</a>
		<a href="https://nanocenter.org/projects">Community Projects</a>
		<a href="https://wiki.nanocenter.org" class="active">Wiki</a>
		<a href="https://blog.nanocenter.org">Blog</a>
		<a href="https://nanocenter.org" class="button">Learn About Nano</a>
</div>
		';
					...
					Html::rawElement( 'div', [ 'id' => 'content', 'class' => 'mw-body',  'role' => 'main' ],
						...
						Html::rawElement( 'div', [ 'id' => 'mw-page-header-links' ],
							$this->getPortlet(
								...
								'nanocenter-namespaces'
							) .
							$this->getPortlet(
								'views',
								$this->pileOfTools['page-primary'],
								'nanocenter-pagetools'
							)
						) .
						$this->getClear() .
...
<div class="section-project-start">
<h1>Create something together 
with the Nano community</h1>
<p><a href="https://nanocenter.org/project/create" class="button">Start your own project</a></p>
</div>
<div class="section-menu">
<div class="section-menu-group">
<a href="http://discord.nanocenter.org"><span class="link-title">Community</span><span class="link-description">Join our Discord</span></a>
<a href="https://blog.nanocenter.org"><span class="link-title">Blog</span><span class="link-description">Nano Center Blog</span></a>
</div>
<div class="section-menu-group">
<a href="https://wiki.nanocenter.org"><span class="link-title">Wiki</span><span class="link-description">Nano Center Wiki</span></a>
<a href="https://nanocenter.org/donate"><span class="link-title">Donate</span><span class="link-description">Support the Nano Center</span></a>
</div>
</div>
<div class="section-copyright">
&copy; '.date('Y').' NanoCenter
</div>
</div>'
			)
		);
	...
	protected function getLogo( $id = 'p-logo', $part = 'both' ) {
		...
		if ( $part !== 'image' ) {
			...
			if ( $language->hasVariants() ) {
				$siteTitle = $language->convert( $this->getMsg( 'nanocenter-sitetitle' )->text() );
			} else {
				$siteTitle = $this->getMsg( 'nanocenter-sitetitle' )->text();
			}
			...
			Html::rawElement( 'div', [ 'id' => 'simpleSearch' ],
				Html::rawElement( 'div', [ 'id' => 'searchInput-container' ],
					$this->makeSearchInput( [
						'id' => 'searchInput',
						'placeholder' => $this->getMsg( 'nanocenter-search-placeholder' )->text(),
					] )
				) .
	...
	protected function getPageToolSidebar() {
		...
		$pageTools .= $this->getPortlet(
			...
			'nanocenter-pageactions'
		);
		$pageTools .= $this->getPortlet(
			'userpagetools',
			$this->pileOfTools['user'],
			'nanocenter-userpagetools'
		);
		$pageTools .= $this->getPortlet(
			'pagemisc',
		...
		return $this->getSidebarChunk( 'page-tools', 'nanocenter-pageactions', $pageTools );
	}
	...
	protected function getUserLinks() {
		...
		if ( isset( $personalTools['userpage'] ) ) {
			$personalTools['userpage']['links'][0]['text'] = $this->getMsg( 'nanocenter-userpage' )->text();
		}
		if ( isset( $personalTools['mytalk'] ) ) {
			$personalTools['mytalk']['links'][0]['text'] = $this->getMsg( 'nanocenter-talkpage' )->text();
		}
		...
		if ( $user->isLoggedIn() ) {
			...
			if ( mb_strlen( $userName ) < $fit ) {
				...
				$dropdownHeader = wfMessage( 'nanocenter-loggedin' )->text();
			}
			$headerMsg = [ 'nanocenter-loggedinas', $user->getName() ];
		} else {
			$dropdownHeader = wfMessage( 'nanocenter-anonymous' )->text();
			$headerMsg = 'nanocenter-notloggedin';
		}
	...
	protected function getPageTools() {
		...
		if ( $namespace >= 0 ) {
			$pileOfTools['pagelog'] = [
				'text' => $this->getMsg( 'nanocenter-pagelog' )->text(),
				'href' => SpecialPage::getTitleFor( 'Log', $title->getPrefixedText() )->getLocalURL(),
				'id' => 't-pagelog'
			];
		}
		$pileOfTools['more'] = [
			'text' => $this->getMsg( 'nanocenter-more' )->text(),
			'id' => 'ca-more',
		...
		if ( $this->data['language_urls'] !== false ) {
			$pileOfTools['languages'] = [
				'text' => $this->getMsg( 'nanocenter-languages' )->escaped(),
				'id' => 'ca-languages',
				'class' => 'dropdown-toggle'
			];
		}
